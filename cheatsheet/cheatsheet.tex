...
\begin{multicols}{4}
  ...
  \textbf{conditions for reaction:}
  \begin{itemize}
    \item molecules must \textbf{collide}
    \item molecules must collide with \textbf{sufficient energy} (activation energy $E_a$)
    \item molecules must collide in \textbf{correct orientation}
  \end{itemize}

  \textbf{activation energy:} reactant molecules must achieve $E_a$ before reaction can occur

  \textbf{catalyst:}
  \begin{itemize}
    \item increases rate by providing \textbf{lower activation energy path}
    \item participates but \textbf{not altered} by reaction
    \item does \textbf{not appear} in overall reaction
    \item avoids high temperature needed for high $E_a$ reactions
  \end{itemize}

  \textbf{factors affecting rate:}
  \begin{itemize}
  ...
  \subsection{measuring reaction rates}

  \textbf{rate of reaction} $\dot{R}$: units $M/s$ ($\frac{mol}{L \cdot s}$)

  for $aA+bB\rightarrow cC+dD$:
  $$\dot{R}=-\frac{1}{a}\frac{d[A]}{dt}=-\frac{1}{b}\frac{d[B]}{dt}=\frac{1}{c}\frac{d[C]}{dt}=\frac{1}{d}\frac{d[D]}{dt}$$

  \textbf{instantaneous rate:} $\frac{d[A]}{dt}$ = slope of tangent on $[A]$ vs. time plot

  rates must be determined \textbf{experimentally}

  \subsection{differential rate laws}

  for $aA+bB \rightarrow \text{products}$:
  $$\dot{R}=-\frac{1}{a}\frac{d[A]}{dt}=-\frac{1}{b}\frac{d[B]}{dt}=k[A]^{m}[B]^{n}$$

  \begin{itemize}
    \item $m$: order w.r.t. A
    \item $n$: order w.r.t. B
    \item $m+n$: overall order
    \item $k$: rate constant (function of temperature)
  \end{itemize}

  \textbf{note:} exponents ($m$, $n$) $\neq$ stoichiometric coefficients ($a$, $b$)

  \textbf{method of initial rates:} determines instantaneous rate at beginning before concentrations change significantly
  ...
  \begin{itemize}
    ...
    \item plot: $\ln[A]$ vs. time, slope = $-ak$
    \item units of $k$: $1/s$
  ...
  \begin{itemize}
    ...
    \item plot: $\frac{1}{[A]}$ vs. time, slope = $ak$
    \item units of $k$: $1/(M\cdot s)$
  ...
  \begin{itemize}
    ...
    \item $E_{a}$: activation energy (J/mol)
    \item $R$: gas constant ($8.314$ J/(mol $\cdot$ K))
  ...
  plot $\ln k$ vs. $\frac{1}{T}$: slope = $-\frac{E_{a}}{R}$
  ...
  \textbf{\ce{CO2}:} triple point 5.1 atm, -56.7$^\circ$C; critical point 72.9 atm, 31$^\circ$C

  \textbf{\ce{H2O}:} polymorphism (ice I, II, III, V, VI, VII); solid-liquid boundary has negative slope (ice less dense than water); triple point 0.006 atm, 0.01$^\circ$C
  ...
  water: $K_b = 0.512$, $K_f = 1.86$ K$\cdot$kg/mol. $K_b$, $K_f$ depend only on solvent, $m$ = molality
